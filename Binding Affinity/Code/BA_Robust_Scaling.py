# -*- coding: utf-8 -*-
"""
Created on Tue Jun  4 13:09:11 2024

@author: marta"

Robust Scaling uses the median and the interquartile range (IQR) for scaling, 
...
def read_smiles_from_csv(file_path):
    ...
    percentile_05 = np.percentile(df['pCHEMBL'], 5)
    percentile_95 = np.percentile(df['pCHEMBL'], 95)
    
    # Cut data at 5th and 95th percentiles
    df_cut = df[(df['pCHEMBL'] >= percentile_05) & (df['pCHEMBL'] <= percentile_95)]
    
    smiles_list = df_cut['SMILES'].tolist()
    y = df_cut['pCHEMBL'].values
    
    # Robust scaling for the target variable
    robust_scaler = RobustScaler()
    y_scaled = robust_scaler.fit_transform(y.reshape(-1, 1)).flatten()
    
    return smiles_list, y_scaled, robust_scaler
...
def train_evaluate_model(X_train_val, y_train_val, X_test, y_test, token_dict_length, n_splits=5):
    ...
    for train_index, val_index in kf.split(X_train_val):
        ...
        y_test_preds += y_pred_scaled
        
        mse = mean_squared_error(y_test, y_pred_scaled)
        r2 = r2_score(y_test, y_pred_scaled)
        ccc = concordance_correlation_coefficient(y_test, y_pred_scaled)
        ...
        if mse < best_run_params['mse']:
            best_run_params.update({'mse': mse, 'r2': r2, 'ccc': ccc, 'preds': y_pred_scaled})
...
file_path = "C:/Users/marta/OneDrive/Backup/Ambiente de Trabalho/Universidade/Licenciatura/3ºano/2º Semestre/Projeto em Engenharia Biomédica/Binding Affinity/Data/data_clean_kop.csv"

# Read SMILES strings and target values from the CSV file
smiles_list, y_scaled, robust_scaler = read_smiles_from_csv(file_path)
...
X_train_val, X_test, y_train_val, y_test = train_test_split(X, y_scaled, test_size=0.15, random_state=42)
...
y_test_preds_original_scale = robust_scaler.inverse_transform(best_run_params['preds'].reshape(-1, 1)).flatten()
y_test_original_scale = robust_scaler.inverse_transform(y_test.reshape(-1, 1)).flatten()
...
plt.title('Actual vs Predicted pCHEMBL')
plt.show()
