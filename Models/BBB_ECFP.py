# -*- coding: utf-8 -*-
"""
Created on Fri Apr  5 18:56:02 2024
...
from sklearn.metrics import roc_auc_score, precision_recall_curve, auc, confusion_matrix, f1_score, precision_score, matthews_corrcoef
...
# Function to convert SMILES to ECFP
def smiles_to_ecfp(smiles, radius=2, n_bits=1024):
    try:
        ...
        ecfp = AllChem.GetMorganFingerprintAsBitVect(mol, radius, n_bits)
...
def reduce_dimensionality(X_train, X_val, X_test, degree=2):
    kpca = KernelPCA(kernel='poly', degree=degree)
    X_train_kpca = kpca.fit_transform(X_train)
    X_val_kpca = kpca.transform(X_val)
    X_test_kpca = kpca.transform(X_test)
    print("Shape of X_train after KPCA:", X_train_kpca.shape)
    print("Shape of X_val after KPCA:", X_val_kpca.shape)
    print("Shape of X_test after KPCA:", X_test_kpca.shape)
    return X_train_kpca, X_val_kpca, X_test_kpca

# Function to calculate average and standard deviation of validation metrics
def calculate_metrics_avg_and_std(history):
    # Retrieve validation accuracy and loss from history
    val_accuracy = history.history['val_accuracy']
    val_loss = history.history['val_loss']
    
    # Calculate the average and standard deviation of validation accuracy and loss
    val_accuracy_avg = np.mean(val_accuracy)
    val_accuracy_std = np.std(val_accuracy)
    val_loss_avg = np.mean(val_loss)
    val_loss_std = np.std(val_loss)
    
    # Print the average and standard deviation of validation accuracy and loss
    ...
    print(f"Average Validation Loss: {val_loss_avg:.4f}")
    print(f"Standard Deviation of Validation Loss: {val_loss_std:.4f}")

# Read data from CSV file
file_path = "C:/Users/marta/OneDrive/Backup/Ambiente de Trabalho/Universidade/Licenciatura/3ºano/2º Semestre/Projeto em Engenharia Biomédica/Code/data_bbb_all.csv"
...
try:
    ...
    ecfps = np.array([smiles_to_ecfp(smile) for smile in smiles if smiles_to_ecfp(smile) is not None])
    ...
    X_train_kpca, X_val_kpca, X_test_kpca = reduce_dimensionality(X_train, X_val, X_test, degree=2)
    ...
    # Calculate and print average and standard deviation of validation metrics
    calculate_metrics_avg_and_std(history)
