# -*- coding: utf-8 -*-
"""
Created on Fri Apr 12 11:17:52 2024
...
# Function to convert SMILES to RDKit Morgan fingerprint
def smiles_to_rd_fp(smiles, radius=2, n_bits=2048):
    try:
        ...
        # Generate RDKit Morgan fingerprint
        rd_fp = AllChem.GetMorganFingerprintAsBitVect(mol, radius, nBits=n_bits)
...
try:
    ...
    bbb = df["bbb"].tolist()
    fps = np.array([smiles_to_rd_fp(smile) for smile in smiles if smiles_to_rd_fp(smile) is not None])
    bbb = np.array(bbb)[:fps.shape[0]]

    # Apply SMOTE
    sm = SMOTE(random_state=42)
    fps_res, bbb_res = sm.fit_resample(fps, bbb)
    print('Original dataset shape %s' % Counter(bbb))
    print('Resampled dataset shape %s' % Counter(bbb_res))

    X_train, X_test, y_train, y_test = train_test_split(fps_res, bbb_res, test_size=0.15, random_state=42)
